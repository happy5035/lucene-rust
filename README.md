...
**27 桶中 Rust 更快 15 桶、>1.5x 仅 2 桶（最差 1.58x）**（优化前同口径为 3–14x 全面落后）：

| 分组 | Rust / Java（p50 比值） |
|---|---|
| term / iterm | **0.43–0.47x / 0.75–0.86x** |
| and / or | **0.35–1.06x / 0.54–1.14x** |
| phrase / prefix | **0.46–1.15x / 0.66–1.55x** |
| wildcard | **1.02–1.26x** |
| terms / termsbig | **0.81–1.58x / 0.70–1.46x**（high 桶已反超） |

五轮根因均为热路径常数工程（无格式/架构变更）：① 词典查找分配税
...
kway 归并 heads 改栈数组快照，去每轮 2 次堆分配）；⑤ 高 df 析取 bitset 化
（df 总和 ≥2048 的 Terms/Or 迭代路径改走 bitset 物化，替代 ~28ns/doc 的
k-way 归并；打分路径 needs_freq 保留析取）。另测过 `target-cpu=native`：
本机全面 0.87–1.0x 负收益，已否决。
...
百万级命中的解码常数项（or high 1.58x、and high 1.53x、phrase high 1.53x、
termsbig low 1.73x、wildcard low 1.41x）。④ 使 terms/termsbig high 相对自身
提速 1.71–1.75x 并转入反超；⑤ 使 or high 相对自身提速 1.26x（3.54ms → 2.80ms，
对 Java 2.00x → 1.58x）。注意：Rust 写侧无后台 merge（5M docs 落 38 段 vs
...
`/tmp/5m1-{rust,java}.txt`、`/tmp/n5m2-rust.txt`、`/tmp/n5m4-rust.txt` 原始表）。
