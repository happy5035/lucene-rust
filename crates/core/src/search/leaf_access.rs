...
pub trait PointsAccess {
    ...
    /// Doc-only intersect: like `intersect` but the visitor receives only
    /// the docID (value decoding skipped for Inside subtrees on disk).
    /// Default: delegates to `intersect`, ignoring values.
    fn intersect_docs(
        &self,
        field: &str,
        low: i64,
        ...
        self.intersect(field, low, high, &mut |_val, doc| visitor(doc as u32))
    }

    /// Field-level bounds from .kdm metadata: (min_value, max_value, doc_count).
    /// None = unknown field or no precomputed bounds (memory side).
    fn field_bounds(&self, _field: &str) -> Option<(i64, i64, u32)> {
        None
    }
}
