...
/// K-way merge multiple shards into one compact shard + downsample shards.
///
/// Reads all docs from each input shard, groups by series_hash across all
/// inputs, merges (sort + dedup), then writes:
...
pub fn merge_cross_shard(
    ...
    // 1. Read all input shards, group by series_hash
    struct CrossSeriesData {
        name: String,
        labels: String,
        points: Vec<(i64, f64)>,
    }
    let mut groups: HashMap<i64, CrossSeriesData> = HashMap::new();
    let mut input_docs = 0usize;
    let mut input_points = 0usize;
    ...
    for shard_dir in input_shard_dirs {
        ...
        for (_doc_base, seg) in reader.leaves() {
            ...
            for (doc_id, gorilla_bytes) in &gorilla_datas {
                ...
                let entry = groups.entry(hash).or_insert_with(|| CrossSeriesData {
                    name,
                    labels: labels_str,
                    points: Vec::new(),
                });
    ...
    // 2. Create 3 output writers
    let mut compact_writer =
        IndexWriter::create(output_compact_dir, v5_schema(), IndexWriterConfig::default())?;
    ...
    // 3. For each group: merge + write compact + downsample
    for (hash, data) in &groups {
        // Sort by time, dedup (keep first occurrence)
        let mut points = data.points.clone();
        points.sort_by_key(|(t, _)| *t);
        points.dedup_by_key(|(t, _)| *t);
        ...
        // Write compact (raw merged series)
        write_series(&mut compact_writer, &data.name, &sorted_labels, &times, &values)?;
        output_docs += 1;

        // Write downsample 5m
        let ds5m_data = downsample(&points, INTERVAL_5M);
        write_downsample_doc(&mut ds5m_writer, &data.name, &data.labels, *hash, &times, &ds5m_data)?;
        ds5m_docs += 1;

        // Write downsample 1h
        let ds1h_data = downsample(&points, INTERVAL_1H);
        write_downsample_doc(&mut ds1h_writer, &data.name, &data.labels, *hash, &times, &ds1h_data)?;
        ds1h_docs += 1;
    }
