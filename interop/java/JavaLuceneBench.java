...
/**
 * Java baseline writer benchmark (Lucene 9.12.3), M1 document shape:
 * one text field "message" indexed with DOCS_AND_FREQS + omitNorms, and stored.
 *
 * Usage: JavaLuceneBench <indexDir> <numDocs> <targetDocBytes> <threads> <seed> [ramBufferMB]
 * Output (stdout, machine-readable):
 *   BENCH elapsed_ms=<m> docs_per_sec=<d> mb_per_sec=<b> indexed_bytes=<n>
 */
public class JavaLuceneBench {
    // Deterministic xorshift64* — keep in sync with the Rust generator (same seed => same corpus).
    ...
    static {
        ...
        for (String w : words) for (int i = 0; i < 40; i++) v.add(w + i); // 2000 distinct terms
    ...
    public static void main(String[] args) throws Exception {
        ...
        int ramMB = args.length > 5 ? Integer.parseInt(args[5]) : 256;
        ...
        try (Directory dir = FSDirectory.open(indexDir)) {
            ...
            try (IndexWriter w = new IndexWriter(dir, cfg)) {
                ...
                System.out.printf(Locale.ROOT,
                    "BENCH elapsed_ms=%d docs_per_sec=%.0f mb_per_sec=%.1f indexed_bytes=%d%n",
                    elapsedMs, docsPerSec, mbPerSec, indexedBytes.get());
            }
        }
    }
}
